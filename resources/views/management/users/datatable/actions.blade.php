@if(isset($permissions['view']) && $permissions['view'])
    <a href="{{ route('management.users.show', ['user' => $id]) }}" class="btn btn-outline-info btn-sm dt-tippy-btn" title="@lang('user.actions.view')">
        <i class="fa fa-eye"></i>
    </a>
@endif
...
@if(isset($permissions['delete']) && $permissions['delete'])
    <a href="javascript:void(0);" class="btn btn-outline-danger btn-sm dt-tippy-btn action-button" title="@lang('user.actions.delete')"
       data-action="{{ route('management.users.destroy', ['user' => $id]) }}"
       data-method="DELETE"
