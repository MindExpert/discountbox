...
{{--        </a>--}}
    <a href="javascript:void(0);" class="dt-tippy-btn" data-bs-toggle="modal" data-bs-target="#view-cover-image-modal-{{$discountBox->id}}" title="@lang('discount_box.view_cover_image')">
        {{--<img src="{{ $discountBox->getFirstMediaUrl('cover_image', 'thumb') }}" alt="" style="object-fit: cover; height: 25px; width: 25px" class="rounded-circle" loading="lazy">--}}
        <img src="{{ $discountBox->getFirstMediaUrl('cover_image', 'thumb') }}" alt="" class="avatar-md h-auto d-block rounded" loading="lazy">
    </a>
@else
    <img src="{{ asset('frontend/assets/img/placeholderx1.png') }}"
         alt="{{$discountBox->serial}}"
         class="avatar-md h-auto d-block rounded dt-tippy-btn"
         loading="lazy"
