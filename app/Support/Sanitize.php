...
class Sanitize
{
     ...
     */
    public const PREG_CLASS_PUNCTUATION = '\x{21}-\x{23}\x{25}-\x{2a}\x{2c}-\x{2f}\x{3a}\x{3b}\x{3f}\x{40}\x{5b}-\x{5d}'.
    ...
    public const PREG_CLASS_NUMBERS = '\x{30}-\x{39}\x{b2}\x{b3}\x{b9}\x{bc}-\x{be}\x{660}-\x{669}\x{6f0}-\x{6f9}'.
    ...
    public const PREG_CLASS_SEARCH_EXCLUDE = '\x{0}-\x{2c}\x{2e}-\x{2f}\x{3a}-\x{40}\x{5b}-\x{60}\x{7b}-\x{bf}\x{d7}\x{f7}\x{2b0}-'.
     ...
     * @var string
     */
    public static string $SEARCH_BLACKLIST = '4r5e';

    /**
     * 	minimum characters to start performing the search
     * @var integer
     */
    public static int $SEARCH_MIN_WORD_LENGTH = 1;

    /**
     * maximum characters used per word search
     * @var integer
     */
    public static int $SEARCH_MAX_WORD_LENGTH = 30;
    ...
    public static function sanitize(?string $string = ''): string
    {
        ...
        $blacklist = self::lowercase(self::$SEARCH_BLACKLIST);
        if (!empty($blacklist)) {
            $blacklist = [$blacklist];
