...
if (! function_exists('upper_arch_values')) {
    function upper_arch_values(): array
    {
        return ['11', '12', '13', '14', '15', '16', '17', '18', '21', '22', '23', '24', '25', '26', '27', '28'];
    }
}

if (! function_exists('lower_arch_values')) {
    function lower_arch_values(): array
    {
        return ['31', '32', '33', '34', '35', '36', '37', '38', '41', '42', '43', '44', '45', '46', '47', '48'];
    }
}

if (!function_exists('currency_from_code')) {
    function currency_from_code($code): Model|Currency|null
    {
        $cacheTag = CacheTagEnum::HELPERS_CURRENCY_FROM_CODE->value;

        return Cache::tags([$cacheTag])
            ->remember("{$cacheTag}:{$code}", 3600, function () use ($code) {
                $code = strtoupper(remove_multiple_spaces($code));

                return  Currency::query()
                    ->select('symbol', 'code', 'precision')
                    ->where('code', $code)
                    ->first();
            });
    }
...
if (! function_exists('lead_percentage_reports_color')) {
    function lead_percentage_reports_color(int|float $percentage, int|float $totalPercentage, ?bool $inverse = false): string
    {
        if ($totalPercentage < 10) {
            $minPercentage = $totalPercentage - 0.5;
            $maxPercentage = $totalPercentage + 0.5;
        } else {
            $minPercentage = $totalPercentage - 5;
            $maxPercentage = $totalPercentage + 5;
        }
